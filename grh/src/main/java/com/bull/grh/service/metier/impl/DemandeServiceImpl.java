...
public class DemandeServiceImpl implements DemandeService {
    ...
    @Override
    public void startTaskDemandeRH(DemandeVO demande) throws ServiceException {
        Task task = taskService.createTaskQuery()
                .processInstanceBusinessKey(demande.getId().toString())
                .taskCandidateGroup("ROLE_RH").singleResult();

        if (task == null) throw new ServiceException();

        taskService.claim(task.getId(), SecurityContextHolder.getContext().getAuthentication().getName());
    }

    @Override
    public void startTaskDemandeOP(DemandeVO demande) throws ServiceException {
        Task task = taskService.createTaskQuery()
                .processInstanceBusinessKey(demande.getId().toString())
                .taskCandidateGroup("ROLE_RH").singleResult();
    ...
    @Override
    public void rejectDemande(DemandeVO demande) throws ServiceException {
        // proceed by completing the task
        Task task = taskService.createTaskQuery()
                .taskDefinitionKey(ProcessConst.DEMANDE_TASK_RH_VALIDATION)
                .taskAssignee(SecurityContextHolder.getContext().getAuthentication().getName())
    ...
    @Override
    public void rejectDemandeAfterAccepting(DemandeVO demande) throws ServiceException {
        // verify if the demand has some candidatures saved if so delete them from the database
    ...
    @Override
    public void completeDemande(DemandeVO demande) throws DemandeHaveNoCandidatureException, ServiceException {
    ...
    @Override
    public void addCandidateToDemand(CandidatVO candidat, DemandeVO demand)
            throws AlreadyHaveCandidatureException, NoDemandeSelectedException {
